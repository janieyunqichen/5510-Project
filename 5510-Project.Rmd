...
Country Fire Authority (CFA) provides statewide fire and related emergency coordination. The key part of the CFA mission is the fire safety and preparedness, therefore the key part of the CFA work includes: 
...
CFA delivers services to the community through a network of over 55,000 volunteers operating out of more than 1,200 rural and urban brigades across Victoria. CFA also employs around 1,000 Professional, Technical and Administrative staff to support service delivery. CFA uses its network of brigades to respond to emergency events, including structure fires, bush and grass fires, road crashes and vehicle fires. CFA brigades also respond alongside Fire Rescue Victoria and other emergency service organizations. 
 ...
 * Hazard classes: Hazard class defines the type of risk for a given area. Each brigade area may contain multiple hazard classes. However, each hazard class has a predefined service delivery standards. There are three predefined hazard classes: 2, 3 and 4. 
...
In this report, we found the most frequently happened fire incident from Country Fire Authority, which is somehow related to the bushfire in 2019/20. We regard the 2019/20 bushfire as a particular case based on the country fire, which was one of the worst in recent times in the world. The season started in early November 2019 in New South Wales, and gradually progressed in Victoria, Western Australia, South Australia, and the Australian Capital Territory. Thousands of fire fighters and volunteers battled the fires, with millions of hectares burned, thousands of properties damaged, and countless numbers of wildlife exposed. In February 2020, the last fires were reportedly extinguished, with torrential rain assisting in putting out the remaining fires. 

This case will be carried out at 3 points: land damage, environmental damage and species damage. So in general, the datasets for analysing the 2019 bushfire contain the information of area of land damage by the type, Melbourne air quality index, the distribution of the threatened species and the effects on them.

The firefighters and the non-operational volunteers did a remarkable job in all the fire incidents, including bushfires. This report will also analyze the structure of the volunteers of CFA in Victoria, Australia.


The following questions will be discussed in this report:

* What is the most frequent fire incident in the year 2015? (CFA Incident Responses)

* How long did it take on average for the CFA brigade to respond to incidents in their own areas for hazard class 2 in Q4 2018/19 and Q4 2019/20?  Also, this will be analysed against the standard time (given in the data for each of the hazard classes). (CFA Emergency Response Time)
...
* What is the volunteer age and serving length structure in 2016? (CFA Volunteer Demographics)

* What is the number of fire detections, extent and land type affected in Australia and Victoria in the Australian bushfire 2020? (Australian Department of Agriculture)

* What was the impact on the air pollution index in Melbourne city in the first trimester 2020?

* How serious are the threatened species affected by the Australian bushfire 2020, and what’s the distribution of them? (Australian Department of Agriculture)
...
For this question we take into account the response time information for the fourth quarter for the years 2018/19 and 2019/20.
...
Let's have a look at the table for the fourth quarter 2019/20.

```{r plotting-data20, message=FALSE, echo=FALSE, warning=FALSE, eval=TRUE}
 data_20%>%
...
Using the summary function, we can look at the summary of minimum, maximum and the mean time it took CFA brigades to respond to incidents within their brigade areas:

```{r tab-summary20, message=FALSE, echo=FALSE, eval=TRUE, warning=FALSE}
data_20 %>%
...
From, the table \@ref(tab:tab-summary20), the **mean** time that it took CFA brigades to respond to incidents in their own CFA brigade areas is **eight minutes and thirty-seven seconds**. This is very close to the target response time for brigades, which is **eight minutes**. 
...
In 87 per cent of emergencies, the community received a fire service within the standard response time, which was broadly consistent with previous quarters. More on this [here](https://news.cfa.vic.gov.au/-/cfa-releases-latest-response-data)
...
Let's look at the table that records information for the year 2018/19 fourth quarter for districts where it took between **seven and twelve minutes** to respond to an incident.



```{r plot-19, message=FALSE, fig.width=13, fig.height=12, fig.align='default', echo=FALSE, eval=TRUE, warning=FALSE}
...
Using the Summary function, let's look at the summary of minimum and maximum amount of time it took Brigades to respond.
...
Using the summary function, we can see that the mean time taken by the CFA brigades to respond to the incidents in their own areas was **eight minutes and fifty-six seconds** as visible from \@ref(tab:tb19Sum) . This is again very close to the target standard which was **eight minutes**.
...
```{r reading-data, message=FALSE, eval=TRUE, echo=FALSE, warning = FALSE}
...
```{r plotIncidents, message=FALSE, fig.width=12, fig.height=8, fig.align='default', fig.cap="Number of incidents", echo=FALSE, eval=TRUE}
ggplot(dat_Q1_2020, aes( x = CFA_Brigade_Area,
                         y = Number_Incidents_total,
                         fill=Number_Incidents_total)) + 
  geom_bar(stat = "identity", 
position = "dodge")+
  theme_base()+
  theme(axis.text.x = element_text(angle = 90 , vjust = 0.5, hjust=0.5), axis.title.x = element_text(vjust=-1.8))+
  xlab("Incidents") +  
 ylab("Districts") +  
...
```{r, eval = TRUE, echo= FALSE, message=FALSE, fig.width=15, fig.height=8, fig.align='default', fig.cap="Number of incidents" }
...
```{r reading-merged, message=FALSE, eval=TRUE, echo=FALSE, warning=FALSE}
...
```{r reading-mergedData, message=FALSE, eval=TRUE, echo=FALSE, fig.width=15, fig.height=10, fig.align='default', fig.cap="Inicdents for the third quarter 2019/20", warning=FALSE}
 ggplot(dat_Q3_2020, aes( x = CFA_Brigade_Area, 
                         y = Number_Incidents_total
                         )) +
geom_col()+
  theme_base()+
...
```{r reading-q4, message=FALSE, eval=TRUE, echo=FALSE, warning=FALSE}
...
```{r plotting, fig.width=12, fig.height=10, fig.align='default', fig.cap="Number of incidents for the fourth quarter 2019/20", echo=FALSE, eval=TRUE}
ggplot(dat_Q4_2020,
...
palette = "Dark2"
  ...
  theme_clean()+
  theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=0.5), axis.title.x = element_text(vjust=-1.8))+
  xlab("Incidents") +  
 ylab("Districts") +  
  ggtitle("Number of incidents") 
```
...
```{r, echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, echo=FALSE}
...
```{r TabNum, eval= TRUE, echo=FALSE, message=FALSE, warning=FALSE}
...
The data used here is the merged data that contains information on all four quarters. For the next steps, we will use the binding_data from above. Let's first visualize data for four quarters.

```{r, eval = TRUE, echo= FALSE, message=FALSE, fig.width=15, fig.height=15, fig.align='default', fig.cap="Number of incidents"}

ggplot(binding_data,
       aes(x = Number_Incidents_total,
           y= CFA_Brigade_Area))+ 
  geom_col()+
  theme_base()+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=0.5), axis.title.x = element_text(vjust=-1.8))+
...
We can see that highest number of incidents are `r max(binding_data$Number_Incidents_total)`, so now using the filter function, which we know from the graph above that Dandenong has had the highest number of incidents.Let's filter for the Dandenong district and see how many incidents took place in each quarter.  
...
binding_data%>%
  select(-X1,
         -`CFA District`)%>%
  filter(CFA_Brigade_Area == "Dandenong" )%>%
...
```{r a, message=FALSE, fig.width=15, fig.height=8, fig.cap="Number of Observations by Year", fig.align='default',eval=TRUE, echo=FALSE, message=FALSE,warning=FALSE}
 ...
 ggplot(fire_total_detects, 
  ...
  geom_text(aes(label = n, size = 1, hjust= 0), show.legend = F)+
  theme_bw()
...
```{r affectedstate, message=FALSE, warning=FALSE, echo=FALSE, eval=TRUE, fig.align='center', fig.show="hold", fig.cap="Affectation by State", eval=TRUE, echo=FALSE, message=FALSE,warning=FALSE}
...
```{r affectedvic, message=FALSE, fig.align='center', fig.show="hold", fig.cap="Land Type affected in Victoria", eval=TRUE, echo=FALSE, message=FALSE,warning=FALSE}
...
The bushfires season had a huge negative impact in the air quality in Victoria, as an example we took data from the Environment Protection Authority in Australia which recorder the PM2.5 (Fine Particulate Matter 2.5) which is an air pollutant concern for the community's health, the higher the rate the worse impact for individuals. In the figure \@ref(fig:airyears) below can be seen the levels for the previous four years, with clearly a huge peak in January 2020. According to Vadourlakis & Et Al, the particles can introduce inflammation in the lungs and even translocate to the bloodstream system, also when having higher temperatures this can increase the rate of mortality. Other places around Australia, specially in New South Wales, had PM2.5 levels over 600.![Link](https://www.mja.com.au/system/files/issues/212_08/mja250511.pdf)
...
```{r figindex, fig.align='center', out.width="70%", message=FALSE, fig.cap="Air Quality Levels", eval=TRUE, echo=FALSE, message=FALSE,warning=FALSE}
...
```{r figmelb, message=FALSE, fig.align='center', fig.cap="Bushfire Season 2019/2020 Air Quality in Melbourne", eval=TRUE, echo=FALSE, message=FALSE,warning=FALSE}
...
```{r datSummary2, message=FALSE, warning=FALSE, echo=FALSE, eval=TRUE, fig.cap="The summary of the affects on the threatened situation by species category"}
...
category_summary <- read_excel("data/protected-species-in-fire-affected-areas-20Jan.xlsx", sheet = 2, range = "B22:J26")%>%
 ...
 p2<-ggplot(category_summary, aes(Threatened, Species, fill= Count_category)) + 
  ...
  labs(x="", y="")+
  theme_ipsum()+
  ggtitle("The summary of the affects on the threatened situation by species category")

ggplotly(p2)
```

According to the plot \@ref(fig:datSummary2), it seems that, for the vulnerable species, it's the plant which occurred the most serious destruction, besides, in terms of the endangered or critically endangered species, the other species like fish, bird, frog and mammals, were not as serious as the plants. In addition, there were 4 migratory species affected because of the bushfire.

Nearly three billion animals: mammals, reptiles, birds, and frogs were killed or displaced by Australia’s devastating 2019-20 bush fires. It’s almost three times an earlier estimate released in January. The breakdown is 143 million mammals, 2.46 billion reptiles, 180 million birds, and 51 million frogs.


# Conclusion

As per the analysis for the question on the brigade's performance, it is safe to say that CFA brigades performed well against the standard delivery service time of eight minutes for the hazard class two incidents. Furthermore, as per the analysis for the number of incidents, we can see that Dandenong appears to be the area with the highest number of incidents across all hazard classes.

The most frequent fire incident is Small vegetation fire (one hectare or less) which contains bushfire, wildfire, or rural fire. In 2016, most volunteers were over 65 years old, and the majority had served for 1-5 years. Hume district had the most CFA volunteers. The percentage of operational and non-operational are about 35% and 65%.

For analyzing the effects on the species, most of the plants have more than 10%, but less than 30% area has been destroyed, with 107 plant species affected. In terms of the threatening condition, the plants suffered the most severe destruction as well. However, the effects on the other species like mammal, frog, and bird species all had similar extent in the destruction, not as severe as the plants.

Nevertheless, many species not currently listed under national environmental law will have had much of their range affected by the fires. In some cases, that impact may mean that these species have become threatened. The Department will be considering assessments of these species soon. Besides, our knowledge about species distributions, fire severity, other threats, and impact on plants and animals is incomplete, which requires us to update the information further.

The Australian government takes the recovery as the national priority, has spent over **$1.6 billion** from the NBRF and **$2.3 billion** total Commonwealth funds on bushfire response, recovery and resilience.
...
* CFA official News https://news.cfa.vic.gov.au/-/cfa-release-latest-response-data

* Department of Health. (n.d.). Retrieved from https://www.health.ny.gov/environmental/outdoors/air/smoke_from_fire.htm#:~:text=All.

* Fattal, A., Kelly, S., Liu, A., Giurco, D. 2016., Waste Fires in Australia: Cause for Concern? Prepared for the Department of Environment, Canberra by the UTS Institute for Sustainable Futures, Sydney.

* Volunteer with CFA. (2021, April 30). Retrieved from https://www.cfa.vic.gov.au/volunteer-careers/volunteers.

* Vardoulakis, S., Jalaludin, B. B., Morgan, G. G., Hanigan, I. C., & Johnston, F. H. (2020). Bushfire smoke: urgent need for a national health protection strategy. The Medical Journal of Australia, 212(8), 349.

* The GIS data of the species of National Environmental significance. Retrieved from: https://www.environment.gov.au/science/erin/databases-maps/snes.

* National Bushfire Recovery Fund. Retrieved from: https://recovery.gov.au/progress-date/funding.

* The aniamls impacted by the bushfire. Retrieved from: https://www.wwf.org.au/news/news/2020/3-billion-animals-impacted-by-australia-bushfire-crisis#gs.2525ta.
