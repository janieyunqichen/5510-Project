...
# How serious the threatened species affected by the Australian bushfire 2020, and what’s the distribution of them?
...
The summary of the affects on species
```{r}
...
overlap_summary <- read_excel("data/protected-species-in-fire-affected-areas-20Jan.xlsx", sheet = 2, range = "B13:J17")%>%
  ...
  mutate(reorder(Species, Count_overlap))
...
category_summary <- read_excel("data/protected-species-in-fire-affected-areas-20Jan.xlsx", sheet = 2, range = "B22:J26")%>%
  ...
  mutate(reorder(Species, Count_category))

ggplot(category_summary,
       aes(Threatened, Count_category,
           fill = Species,
           label = Species))+
  geom_col(position = "fill",
            alpha = 0.8)+
  ...
  xlab("Threatened")+
  ylab("Count")+
  ggtitle("The summary of the affects on the threatened situation")+
  theme_bw()
ggplotly()
```
